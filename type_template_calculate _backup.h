		...
		namespace TTC { //type template calculate
			...
			public:
				...
				struct have : std::conditional< K::template have<C>::value,
					...
					instant_type<get_have,parameter_stack_<T...>,C>
				>::type::type {
				...
				template<typename C> struct get {
					typedef typename std::conditional<
						...
						instant_type<get_get, K, C>,
						instant_type<get_get, parameter_stack_<T...>, C>
					>::type::type type;
				};
				...
				template<typename C, typename P> struct equal {
					typedef typename std::conditional<
						...
						depute_instant<
							...
							instant_type< parameter_stack_<T...>::template equal,C,P>
						>
					>::type::type type;
				};
			};
			...
			template<typename T, typename ...AT> struct if_ {
				template<typename inp_t> class input {
					...
					typedef typename std::conditional<
						...
						instant_type<statement_<AT...>::template input,para>,
						itself<para>
					>::type::type tem_type;
				public:
				...
				template<typename ...AK> struct else_ {
					template<typename inp_t> class input {
						...
						typedef typename std::conditional<
							get_extract_type_<T, inp_t>::type::value,
							instant_type<statement_<AT...>::template input, para>,
							instant_type<statement_<AK...>::template input, para>
						>::type::type temp_type;
					public:
			...
			public:
				typedef typename std::conditional<
					...
					instant_type< out_put::template get_attribute,control_::return_ >,
			...
			template<typename T, typename ...K> struct while_ {
				template<typename in> class input {
					...
					typedef typename std::conditional<
						...
						instant_type< statement_<K...>::template input, in_type>,
						itself< in_type >
					>::type::type out_1;
				...
				public:
					typedef typename std::conditional<
						...
						instant_type< while_<T,K...>::template input, out_type>, 
						itself<out_type>
					>::type::type type;
				};
			};
